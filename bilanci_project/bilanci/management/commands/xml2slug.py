...
class Command(BaseCommand):
    ...
    def save_codice_voce(self, voce_slug, voce_cod, quadro_cod, colonna_cod, denominazione_voce, denominazione_colonna):
        ...
        try:
            ...
            self.logger.error(
                u"Voce with slug:{0} is not present in DB and Codice voce cannot be saved".format(voce_slug))
            return
        ...
        if self.dryrun is False:
            ...
            if created:
                self.logger.debug(
                    u"CodiceVoce created: {0}:{1}-{2}-{3}".format(voce.slug, voce_cod, quadro_cod, colonna_cod))
                if voce_slug not in self.added_voce_slug:
                    self.added_voce_slug.append(voce_slug)

            else:
                self.logger.debug(
                    u"CodiceVoce already present: {0}:{1}-{2}-{3}".format(voce.slug, voce_cod, quadro_cod, colonna_cod))
    ...
    def get_voci_titolo(self, cod_quadro, quadro_denominazione, slug_startswith=None):
        ...
        lambda_cod_denominazione_slug = lambda x: x[4] == cod_quadro and quadro_denominazione.lower() in x[
            0].lower() and x[6].startswith(slug_startswith)
    ...
    def fill_in_voci(self):
        ...
        voci_q4_impegni = self.get_voci_titolo(cod_quadro='04',
                                               quadro_denominazione='QUADRO 4 - SPESE CORRENTI - (A) - IMPEGNI',
                                               slug_startswith='consuntivo-spese-impegni-')
        ...
        self.convert_voci_slugs(cod_quadro='04',
                                quadro_denominazione='QUADRO 4 - SPESE CORRENTI - (B) - PAGAMENTI IN CONTO COMPETENZA',
                                old_subslug='impegni', new_subslug='pagamenti-in-conto-competenza')
        self.convert_voci_slugs(cod_quadro='04',
                                quadro_denominazione='QUADRO 4 - SPESE CORRENTI - (C) - PAGAMENTI IN CONTO RESIDUI',
                                old_subslug='impegni', new_subslug='pagamenti-in-conto-residui')

        # convert slugs for q5 impegni, conto competenza, residui
        self.convert_voci_slugs(cod_quadro='05',
                                quadro_denominazione='QUADRO 5 - SPESE IN CONTO CAPITALE - (A) - IMPEGNI',
                                old_subslug='impegni-spese-correnti', new_subslug='impegni-spese-per-investimenti')
        self.convert_voci_slugs(cod_quadro='05',
                                quadro_denominazione='QUADRO 5 - SPESE IN CONTO CAPITALE - (B) - PAGAMENTI IN CONTO COMPETENZA',
                                old_subslug='impegni-spese-correnti',
                                new_subslug='pagamenti-in-conto-competenza-spese-per-investimenti')
        self.convert_voci_slugs(cod_quadro='05',
                                quadro_denominazione='QUADRO 5 - SPESE IN CONTO CAPITALE - (C) - PAGAMENTI IN CONTO RESIDUI',
                                old_subslug='impegni-spese-correnti',
    ...
    def fill_in_colonne(self):
        ...
        colonne_q4_impegni = self.get_colonne_titolo(cod_quadro='04',
                                                     quadro_denominazione='QUADRO 4 - SPESE CORRENTI - (A) - IMPEGNI')
        colonne_q5_impegni = self.get_colonne_titolo(cod_quadro='05',
                                                     quadro_denominazione='QUADRO 5 - SPESE IN CONTO CAPITALE - (A) - IMPEGNI')
        ...
        self.convert_colonne_slugs(cod_quadro='04',
                                   quadro_denominazione='QUADRO 4 - SPESE CORRENTI - (B) - PAGAMENTI IN CONTO COMPETENZA',
                                   old_subslug='impegni', new_subslug='pagamenti-in-conto-competenza')
        self.convert_colonne_slugs(cod_quadro='04',
                                   quadro_denominazione='QUADRO 4 - SPESE CORRENTI - (C) - PAGAMENTI IN CONTO RESIDUI',
        ...
        # convert slugs for q5 impegni, conto competenza, residui
        self.convert_colonne_slugs(cod_quadro='05',
                                   quadro_denominazione='QUADRO 5 - SPESE IN CONTO CAPITALE - (B) - PAGAMENTI IN CONTO COMPETENZA',
    ...
    def handle(self, *args, **options):
        ...
        self.logger.info("Deleting previous CodiceVoce for considered bilancio, year")
        ...
        if is_mapping_incomplete:
            ...
            gdocs.write_to_csv(path_name='bilancio_{0}_{1}'.format(self.bilancio_type, self.bilancio_year),
                               contents={'voci': self.voci, 'colonne': self.colonne})
            ...
            if n_colonne_quadro == 0:
                    ...
                    if denominazione_voce.lower() == 'totale':
                        ...
                        for colonna in colonne_quadro:
                            ...
                            if denominazione_colonna.lower() != 'totale':
                                ...
                                self.save_codice_voce(colonna_voce_slug, voce_cod, quadro_cod, colonna_cod,
                                                      denominazione_voce, denominazione_colonna)
                    else:
                ...
                # ony one column is needed: the last one (current year value) so no slug must be provided in the gdoc
                    ...
                    for colonna in colonne_quadro:
                        ...
                        self.save_codice_voce(colonna_voce_slug, voce_cod, quadro_cod, colonna_cod, denominazione_voce,
                                              denominazione_colonna)
        ...
        if len(not_mapped_slugs) > 0:
            ...
            self.logger.warning(
                "THERE ARE {0} VOCE SLUG FROM BILANCIO TREE HAS NOT BEEN MAPPED (Bilancio subtree has {1} nodes) ".format(
                    len(not_mapped_slugs), len(tree_slugs)))
