...
\    """Patches city_year_budget_dict with sums for Spese correnti and
    Investimenti

    Args:
        city_year_budget_dict (dict): dictionary containing the budget from 
        couch
    ...
    def somma_funzioni(cor_dict, inv_dict, somma_dict):
        ...
        for k in cor_dict:
            ...
            if isinstance(cor, dict) and isinstance(inv, dict):
                ...
                print "WARNING Errore in somma funzioni: chiave {0}. " \
                "Skipping".format(
                    k)
                continue
            else:
    ...
    if 'consuntivo' in certificati_to_import:
        if 'consuntivo' in city_year_budget_dict and \
                        ...
                        'Impegni' in city_year_budget_dict['consuntivo'][
                    'SPESE']:
            cons_spese_cassa_dict = \
            city_year_budget_dict['consuntivo']['SPESE']['Impegni']
        ...
        if 'consuntivo' in city_year_budget_dict and \
            ...
            cons_spese_impegni_dict = \
            city_year_budget_dict['consuntivo']['SPESE']['Cassa']
...
def map_tree(territorio, populations, city_budget, city_years, partial_import,
             tree_node_slug, couch_path, voci_dict):
    results = []
    city_finloc = territorio.cod_finloc
    for year, certificati_to_import in city_years.iteritems():
        if str(year) not in city_budget:
            # logger.warning(u" {} - {} not found. Skip".format(city_finloc,
            # year))
        ...
        if partial_import is True:
            ...
            for k in couch_path:
                try:
                    ...
                    print "couch path:{} not present for {}, anno:{}".format(
                        couch_path, territorio.cod_finloc,
                        str(year))
                    path_not_found = True
                    break

            # if data path is found in the couch document, write data into
            # postgres db
            if not path_not_found:
                city_year_node_tree_patch = tree_models.make_tree_from_dict(
                    city_year_budget_node_dict, voci_dict,
                    path=[tree_node_slug],
            ...
            for tipo_bilancio in certificati_to_import:
                certificato_tree = tree_models.make_tree_from_dict(
                    city_year_budget_dict[tipo_bilancio], voci_dict,
                    path=[unicode(tipo_bilancio)],
                    population=population
                )
...
class Command(BaseCommand):
    option_list = BaseCommand.option_list + (
        make_option('--dry-run',
                    ...
                    action='store_true',
                    default=False,
                    help='Set the dry-run command mode: nothing is written in '
                    'the couchdb'),
        make_option('--complete',
                    dest='complete',
                    action='store_true',
                    default=False,
                    help='After data import calculate indicators and updates '
                    'opendata zip file'),
        make_option('--years',
                    dest='years',
                    default='',
                    help='Years to fetch. From 2002 to 2012. Use one of this '
                    'formats: 2012 or 2003-2006 or 2002,2004,2006'),
        make_option('--cities',
                    dest='cities',
                    default='',
                    help='Cities codes or slugs. Use comma to separate '
                    'values: Roma,Napoli,Torino or  "All"'),
        make_option('--couchdb-server',
                    dest='couchdb_server',
                    default=settings.COUCHDB_DEFAULT_SERVER,
                    help='CouchDB server alias to connect to (staging | '
                    'localhost). Defaults to staging.'),
        make_option('--create-tree',
                    dest='create_tree',
                    action='store_true',
                    default=False,
                    help='Force recreating simplified tree leaves from csv '
                    'file or gdocs (remove all values)'),
        make_option('--force-google',
                    dest='force_google',
                    action='store_true',
                    default=False,
                    help='Force reloading mapping file and simplified '
                    'subtrees leaves from gdocs (invalidate the csv cache)'),
        make_option('--tree-node-slug',
                    dest='tree_node_slug',
                    default=None,
                    help='Voce slug of the tree model to start the import '
                    'from. Example: consuntivo-entrate-imposte-e-tasse'),
        make_option('--couch-path',
                    dest='couch_path_string',
                    default=None,
                    help='CouchDB keys sequence (CSV) to identify the import '
                    'starting point. '
                         'Must be specified together with the treee-node-slug '
                         'option. '
        ...
        make_option('--append',
                    ...
                    action='store_true',
                    default=False,
                    help='Use the log file appending instead of overwriting ('
                    'used when launching shell scripts)'),
    )

    help = 'Import values from the simplified couchdb database into a ' \
    'Postgresql server'
    dryrun = False
    ...
    # if the import is partial root_treenode is the root node of the sub-tree
    #  to be imported
    root_treenode = None
    ...
    def create_voci_tree(self, force_google):
        ...
        simplified_leaves = gdocs.get_simplified_leaves(
    ...
    def create_voci_preventivo_tree(self, simplified_leaves):
        ...
        for subtree_slug, subtree_denominazione in subtrees.items():
            ...
            for leaf_bc in simplified_leaves[subtree_slug]:
                # add this leaf to the subtree, adding all needed
                # intermediate nodes
    ...
    def create_voci_consuntivo_tree(self, simplified_leaves):
        ...
        subtrees = OrderedDict([
            ('consuntivo-entrate', {
                ...
                'sections': [u'Accertamenti',
                             u'Riscossioni in conto competenza',
                             u'Riscossioni in conto residui',
                             u'Cassa']
            }),
            ('consuntivo-spese', {
                'denominazione': u'Consuntivo spese',
                'sections': [u'Impegni', u'Pagamenti in conto competenza',
                             u'Pagamenti in conto residui', u'Cassa']
            }),
        ])
    ...
    def add_leaf(self, breadcrumbs, subtree_node, section_slug=''):
        ...
        for item in bc:
            if current_node.get_children().filter(
                    denominazione__iexact=item).count() == 0:

                slug = u"{0}-{1}".format(prefix_slug, u"-".join(
                    slugify(unicode(i)) for i in bc[0:bc.index(item) + 1]))
                node = Voce(denominazione=item, slug=slug)
                node.insert_at(current_node, save=True, position='last-child')
                if bc[-1] == item:
                    return
            else:
                node = current_node.get_children().get(
                    denominazione__iexact=item)
    ...
    def set_years(self, years):
        ...
        if "-" in years:
            ...
            years_list = [int(y.strip()) for y in years.split(",") if
                          settings.APP_START_YEAR <= int(
                              y.strip()) <= settings.APP_END_YEAR]
    ...
    def set_cities(self, cities_codes):
        ...
        territorio_finlocs = {t['cod_finloc'][-10:]: t['slug'] for t in
                              Territorio.objects.filter(
                                  territorio="C").order_by('slug').values(
                                  'cod_finloc', 'slug')}
        ...
        for numb in finloc_numbers:
            ...
            if numb in territorio_finlocs.keys():
                ...
                self.logger.warning(
                    u"City with codfinloc:'{}' not found in Postgres DB, "
                    u"skip".format(
                        numb))
    ...
    def checks_partial_import(self, tree_node_slug, couch_path_string):
        # based on the type of import set the type of bilancio that is
        # considered
        # sets branches of somma funzioni considered by the import

        self.partial_import = True

        # depending on tree node slug, couch path string sets considered tipo
        #  bilancio
        ...
        self.root_descendants = self.root_treenode.get_descendants(
            include_self=True)
    ...
    def prepare_for_import(self):
        ##
        # prepare_for_import
        # 1) creates the import_set: the complete dict of cities, years and
        # tipo bilancio that will be imported by the
        #    task
        # 2) creates values_to_delete: a queryset that includes all
        # ValoriBilancio
        #    that correspond to the bilancio selected by the import
        # 3) gets the info about Xml import and removes the keys relative to
        # cities, years and tipo_bilancio
        #    that have been imported via Xml
        # 4) excludes from values_to_delete the values of bilancio imported
        # via XML: they won't be deleted
        # 5) fills somma_funzioni_slug_baseset with a dict that associates
        # the slug of the root node of a
        #    somma-funzioni branch with the set of slugs needed to create it

        # creates a dict with year as a key and value: a list of
        # considered_bilancio_type(s)
        years_dict = OrderedDict(
            (year, self.considered_tipo_bilancio) for year in self.years)

        # creates a dict in which for each city considered the value is the
        # previous dict
        self.import_set = OrderedDict(
            (territorio, years_dict) for territorio in self.cities)

        # construct values_to_delete
        values_to_delete = ValoreBilancio.objects.filter(
            territorio__in=self.cities, anno__in=self.years)
        if self.partial_import:
            values_to_delete = values_to_delete.filter(
                voce__in=self.root_descendants)

        # get data about ImportXml: if there is data that has been imported
        # from XML for a city/ year
        # then the couch import must NOT overwrite that data
        self.imported_xml = ImportXmlBilancio.objects. \
            filter(territorio__in=self.cities, anno__in=self.years,
                   tipologia__in=self.considered_tipo_bilancio). \
            order_by('territorio', 'anno')

        if len(self.imported_xml) > 0:
            for i in self.imported_xml:
                self.logger.warning(
                    "BILANCIO:{} YEAR:{} CITY:{} will have to be reimported "
                    "again: it was imported with xml". \
                        format(i.tipologia.title(), i.anno,
                               i.territorio.denominazione))

        # deletes ValoriBilanci that will be imported afterwards: this speeds
        #  up the import
        if self.partial_import:
            self.logger.info(
                "Deleting values for selected cities, years and subtree")
        else:
            self.logger.info("Deleting values for selected cities, years")

        if not self.dryrun and ValoreBilancio.objects.all().count() > 0:
            if self.partial_import is False and self.cities_param.lower() == \
            'all':
                # sql query to delete all values in ValoreBilancio table:
                # this should cut the time
    ...
    def handle(self, *args, **options):
        ...
        if tree_node_slug and couch_path_string is None or couch_path_string \
        and tree_node_slug is None:
            self.logger.error(
                "Couch path and tree node must be both specified. Quitting")
            exit()
        ...
        and len(
                self.cities) > 4000:
            self.logger.error(
                "DEBUG settings is True, task will fail. Disable DEBUG and "
                "retry")
            exit()
        ...
        # considering years,cities and limitations set creates a
        # comprehensive map of all bilancio to be imported,
        ...
        for territorio, city_years in self.import_set.iteritems():
            populations = {year: territorio.nearest_valid_population(year)[1]
                           for year, budgets in city_years.iteritems()}
            city_finloc = territorio.cod_finloc
            # get all budgets data for the city
            city_budget = self.couchdb.get(city_finloc)
            # self.logger.info("Execution time for couch get: %.2gs seconds"
            # % (time.clock()-start_time))

            if city_budget is None:
                # if city budget is not found, try again taking out
                # apostrophe and re-slugging, this deals with
                ...
                if "'" in territorio.nome:
                    ...
                    city_finloc_noapostrophe = u"{}--{}".format(
                        slugify(nome_senza_apostrofo), finloc_number).upper()
                    city_budget = self.couchdb.get(city_finloc_noapostrophe)

                    if city_budget is None:
                        self.logger.warning(
                            u"Document '{}' or '{}' not found in couchdb "
                            u"instance. Skipping.".format(
                                city_finloc,
                    ...
                    self.logger.warning(
                        u"Document '{}' not found in couchdb instance. "
                        u"Skipping.".format(
                            city_finloc))
                    continue

            self.logger.debug(u"City of {0}".format(city_finloc))
            # map the Couch document tree into memory struct that will be
            # written on Postgres DB later
            params.append((
                territorio, populations, city_budget, city_years,
                self.partial_import, tree_node_slug, self.couch_path,
                self.voci_dict))

            if len(params) == 100:
                self.logger.info(
                    u"Reached city of '{0}', continuing...".format(city_finloc))
                maptree_results = [pool.apply_async(map_tree, p) for p in
                                   params]
                params = []
                ...
                for mtr in maptree_results:
                    ...
                    for result in results:
                        ...
                        if self.dryrun is False:
                            tree_models.write_tree_to_vb_db(territorio, year,
                                                            certificato_tree,
                                                            self.voci_dict)
        ...
        if self.cities_param.lower() != 'all':
            for bilancio_xml in self.imported_xml:
                self.logger.info(
                    "IMPORTANT: Re-import XML bilancio {},{},{}".format(
                        bilancio_xml.territorio, bilancio_xml.anno,
            ...
            if not os.path.exists(xml_path):
                self.logger.error(
                    "XML folder {} not found! Created now".format(xml_path))
                os.makedirs(xml_path)

            xml_files = [f for f in listdir(xml_path) if
                         isfile(join(xml_path, f))]
            for f in xml_files:
            ...
            if len(xml_files) != len(self.imported_xml):
                self.logger.error(
                    "Found {} Xml files compared to {} objs in ImportXML "
                    "table in DB!!".format(
                        len(xml_files),
                        len(self.imported_xml)))
        if len(self.cities_not_found) > 0:
            self.logger.warning(
                "Following COD FINLOC NOT FOUND and not imported:{}".format(
                    ",".join(self.cities_not_found)))

        if complete and not self.dryrun and not self.partial_import:

            ##
            # complete the import with medians,
            # indicators and update opendata (zip files)
            ##

            self.logger.info(u"Update indicators medians")
            call_command('data_completion', verbosity=2, years=options['years'],
                         cities=options['cities'],
                         interactive=False)

            email_utils.send_notification_email(
                msg_string="Couch2pg, update opendata, indicators and medians "
                           "has finished.")

        else:
            email_utils.send_notification_email(
                msg_string="Couch2pg has finished.")
