...
class Command(BaseCommand):
            ...
            for previous_element in table_html.previous_elements:
                if previous_element.name == "div" and previous_element.get('class'):
                    if previous_element.get('class')[0] == 'acentro':
                        ...
                        if len(contents) == 2:
                            table_data['meta']['titolo'] = previous_element.contents[1].text.replace(
                                "(gli importi sono espressi in euro)", '').strip(' \t\n\r')
                            break
    ...
    def handle_scrape(self, *args, **options):
        ...
        try:
            ...
            self.logger.error(msg="Impossible to open the file:%s" % settings.LISTA_COMUNI_PATH)
            return
        ...
        for anno in anni_considerati:
            ...
            for comune in lista_comuni:
                ...
                for quadro in quadri_considerati:
                    url_prev[quadro] = settings.PATH_PREVENTIVI % (anno, comune_slug, quadro)
                    url_cons[quadro] = settings.PATH_CONSUNTIVI % (anno, comune_slug, quadro)

                scrape_list[anno][comune_slug]['P']['url'] = url_prev
                scrape_list[anno][comune_slug]['C']['url'] = url_cons
